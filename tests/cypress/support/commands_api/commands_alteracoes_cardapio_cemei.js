...
// Uma entrada por operacao publicada no Swagger de CEMEI.
export const rotasCardapioCemei = {
	...
	tomar_ciencia: { metodo: 'PATCH', caminho: '{uuid}/terceirizada-toma-ciencia/' },
}

Cypress.Commands.add('requisitar_alteracoes_cardapio_cemei', (operacao, opcoes = {}) => {
	const rota = rotasCardapioCemei[operacao]
	if (!rota) throw new Error(`Operacao CEMEI desconhecida: ${operacao}`)
	if (rota.caminho.includes('{uuid}') && !opcoes.uuid) throw new Error('Informe o UUID da solicitacao CEMEI.')
	const caminho = rota.caminho.replace('{uuid}', opcoes.uuid).replace('{filtro}', opcoes.filtro || 'sem_filtro')
	return cy.request({
		method: rota.metodo,
		url: `${Cypress.config('baseUrl')}api/alteracoes-cardapio-cemei/${caminho}`,
		qs: opcoes.query,
		body: opcoes.dados,
