...
def cria_escola(nome: str, iniciais_tipo: str, dre: DiretoriaRegional, codigo_eol: str, tipo_gestao=None) -> Escola:
...
def cria_solicitacao_com_log(escola: Escola, usuario, status_workflow: str = None, mes=None, ano=None):
    """
...
@pytest.mark.parametrize("status_bloco", [
    "TODOS_OS_LANCAMENTOS",
    SolicitacaoMedicaoInicialWorkflow.MEDICAO_APROVADA_PELA_CODAE,
])
def test_dashboard_dados_ordenados(api_client_usuario_codae, dre, tipo_gestao, status_bloco):
    """
    ...
    emef_1 = cria_escola("EMEF JOAO CARLOS DA SILVA BORGES, PROF.", "EMEF", dre, "100001", tipo_gestao)
    emef_2 = cria_escola("EMEF PERICLES EUGENIO DA SILVA RAMOS", "EMEF", dre, "100002", tipo_gestao)

    # Grupo 1 - CEI (inclui mais itens para validar alfabético)
    cei_diret_1 = cria_escola("CEI DIRET MARIA", "CEI DIRET", dre, "200001", tipo_gestao)
    ...
    ceu_emef_1 = cria_escola("CEU EMEF ROSA PARKS", "CEU EMEF", dre, "500003", tipo_gestao)
    ...
    escolas = [
        emef_1, emef_2,
        cei_diret_1, cei_diret_0,
        ceu_cei_1, ceu_cei_2,
        cci_cips_1, cci_cips_2,
        cemei_1, ceu_cemei_1,
        emei_1, ceu_emei_1,
        emefm_1, cieja_1,
    ...
    assert bloco is not None, f"Bloco {status_bloco} não encontrado. Disponíveis: {[b.get('status') for b in data['results']]}"

    dados = bloco.get("dados", [])
    total = bloco.get("total")
    assert total >= len(escolas), f"Esperava total >= {len(escolas)}, veio {total}. Payload: {data['results']}"
    assert len(dados) >= len(escolas), f"Esperava pelo menos {len(escolas)} itens em dados, obteve {len(dados)}. Payload bloco: {bloco}"
