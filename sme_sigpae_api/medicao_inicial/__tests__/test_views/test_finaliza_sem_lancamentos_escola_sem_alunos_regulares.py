...
@pytest.mark.usefixtures("client_autenticado_da_escola_cmct", "escola_cmct")
class TestUseCaseFinalizaMedicaoSemLancamentosEscolaSemAlunosRegulares:
    def setup_usuario(self):
        self.usuario = UsuarioFactory.create(email="system@admin.com")

    def setup_periodos_escolares(self, escola_cmct):
    ...
    def setup_inclusao_continua_programas_projetos(self, escola_cmct):
        self.inclusao_continua = InclusaoAlimentacaoContinuaFactory.create(
            ...
            rastro_terceirizada=escola_cmct.lote.terceirizada,
    ...
    def setup_medicao_inicial(self, escola_cmct):
        self.solicitacao_medicao_inicial = SolicitacaoMedicaoInicialFactory.create(
            escola=escola_cmct, mes="05", ano="2025"
        )
    ...
    def setup_kit_lanche(self, escola_cmct):
    ...
    def setup_testes(self, escola_cmct):
        self.setup_usuario()
        self.setup_periodos_escolares(escola_cmct)
        self.setup_tipos_alimentacao()
        self.setup_motivos_inclusao_continua()
        self.setup_grupos_medicao()
        self.setup_inclusao_continua_programas_projetos(escola_cmct)
        self.setup_medicao_inicial(escola_cmct)

    def test_finaliza_medicao_sem_lancamentos_erro_programas_e_projetos(
        self, client_autenticado_da_escola_cmct, escola_cmct
    ):
        self.setup_testes(escola_cmct)
        data_update = {
            "escola": str(escola_cmct.uuid),
    ...
    def test_finaliza_medicao_sem_lancamentos_erro_kit_lanche_nao_pode_finalizar(
        ...
        self.setup_testes(escola_cmct)
        self.setup_kit_lanche(escola_cmct)
        data_update = {
            "escola": str(escola_cmct.uuid),
            "com_ocorrencias": False,
            "finaliza_medicao": True,
