...
from sme_sigpae_api.medicao_inicial.models import ValorMedicao, Medicao
from sme_sigpae_api.dieta_especial.models import LogQuantidadeDietasAutorizadasCEI, ClassificacaoDieta
from sme_sigpae_api.medicao_inicial.api.serializers_create import SolicitacaoMedicaoInicialCreateSerializer

pytestmark = pytest.mark.django_db


def test_nao_cria_valores_medicao_cei_sem_faixa_etaria(
    solicitacao_medicao_inicial_sem_valores, escola, periodo_escolar, categoria_dieta_b, categoria_dieta_a
):
    baker.make("Aluno", escola=escola, periodo_escolar=periodo_escolar)
    baker.make("CategoriaMedicao", nome="DIETA ESPECIAL - TIPO A - ENTERAL / RESTRIÇÃO DE AMINOÁCIDOS")
    ...
    serializer.cria_valores_medicao_logs_dietas_autorizadas_cei(solicitacao_medicao_inicial_sem_valores)
...
def test_cria_valores_medicao_cei_com_faixa_etaria(
    solicitacao_medicao_inicial_sem_valores, escola, periodo_escolar, faixa_etaria, categoria_dieta_b, categoria_dieta_a
):
    baker.make("Aluno", escola=escola, periodo_escolar=periodo_escolar)
    baker.make("CategoriaMedicao", nome="DIETA ESPECIAL - TIPO A - ENTERAL / RESTRIÇÃO DE AMINOÁCIDOS")
