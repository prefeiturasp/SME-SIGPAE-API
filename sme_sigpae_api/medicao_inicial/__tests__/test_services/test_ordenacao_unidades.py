...
def test_solicitacoes_ordenacao_unidades(
    api_client_usuario_codae, dre, tipo_gestao, status_bloco
):
    """
    Valida 'dados' após chamada de método de ordenação de solicitações por grupos/subtipos e por nome dentro do subtipo.
    """
    ...
    dados = ordenar_unidades(SolicitacaoMedicaoInicial.objects.all())
    ...
    for i, (tipo_esp, nome_esp) in enumerate(ordem_esperada):
        assert (
            dados[i].escola.tipo_unidade.iniciais == tipo_esp
        ), f"Posição {i}: esperado tipo '{tipo_esp}', obtido '{dados[i].escola.tipo_unidade}'. Item: {dados[i]}"
        assert (
            dados[i].escola.nome == nome_esp
        ), f"Posição {i}: esperado escola '{nome_esp}', obtido '{dados[i].escola.nome}'. Item: {dados[i]}"
