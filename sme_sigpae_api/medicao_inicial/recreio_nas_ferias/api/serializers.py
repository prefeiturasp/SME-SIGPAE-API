...
class RecreioNasFeriasUnidadeParticipanteSerializer(serializers.ModelSerializer):
    lote = serializers.SlugRelatedField(
        slug_field="uuid", queryset=Lote.objects.all(), write_only=True
    )
    unidade_educacional = serializers.SlugRelatedField(
        slug_field="uuid", queryset=Escola.objects.all(), write_only=True
    )

    lote_obj = LoteSerializer(source="lote", read_only=True)
    unidade_educacional_obj = EscolaSerializer(
        source="unidade_educacional", read_only=True
    )
    ...
    def get_tipos_alimentacao(self, obj):
        categorias = {}
        qs = obj.tipos_alimentacao.select_related(
            "tipo_alimentacao", "categoria"
        ).order_by("tipo_alimentacao__posicao")
        for ta in qs:
            nome_cat = (
...
class RecreioNasFeriasCreateSerializer(serializers.ModelSerializer):
    ...
    def _build_unidade_tipos_instances(
        self, unidade, tipos_por_categoria, categorias_map
    ):
        instances = []
    ...
    def create(self, validated_data):
        ...
        for unidade_data in unidades_data:
            ...
            tipo_alimentacao_instances_to_create.extend(
                self._build_unidade_tipos_instances(
                    unidade, tipos_por_categoria, categorias_map
                )
            )

        if tipo_alimentacao_instances_to_create:
            RecreioNasFeriasUnidadeTipoAlimentacao.objects.bulk_create(
                tipo_alimentacao_instances_to_create
            )
    ...
    def update(self, instance, validated_data):
        ...
        if unidades_data is not None:
            ...
            for unidade_data in unidades_data:
                tipos_inscritos = (
                    unidade_data.pop("tipos_alimentacao_inscritos", []) or []
                )
                tipos_colaboradores = (
                    unidade_data.pop("tipos_alimentacao_colaboradores", []) or []
                )
                tipos_infantil = (
                    unidade_data.pop("tipos_alimentacao_infantil", []) or []
                )

                unidade = RecreioNasFeriasUnidadeParticipante.objects.create(
                    recreio_nas_ferias=instance, **unidade_data
                )
