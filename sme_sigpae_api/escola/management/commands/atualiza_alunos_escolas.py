...
class Command(BaseCommand):
    ...
    def get_todos_os_registros(self):
        ...
        escolas = list(Escola.objects.exclude(tipo_unidade__iniciais='ESC.PART.').values_list("codigo_eol", flat=True))
    ...
    def _atualiza_todas_as_escolas_d_menos_2(self):
        ...
        escolas = {e.codigo_eol: e for e in Escola.objects.exclude(tipo_unidade__iniciais='ESC.PART.')}
    ...
    def _atualiza_todas_as_escolas_d_menos_1(self):
        escolas = Escola.objects.exclude(tipo_unidade__iniciais='ESC.PART.').prefetch_related("aluno_set")
        proximo_ano = datetime.date.today().year + 1
