...
from sme_sigpae_api.escola.utils_montar_planilha_matriculados import build_xlsx_alunos_matriculados, monta_celulas_faixas_periodos, montar_cabecalho_padrao, montar_faixas_etarias, style_range
...
def test_build_xlsx_alunos_matriculados(dados_planilha_alunos_matriculados):
    ...
    assert rows[0] == ('Relatório SIGPAE - Alunos Matriculados', None, None, None, None, None, None)
    assert rows[3] == ('Solicitado por Faker usuario, impresso em 28/01/2025', None, None, None, None, None, None)
    assert rows[4] == ('DRE', 'Lote', 'Tipo de unid.', 'Unid. educacional', 'Tipo de Turma', 'Período', 'Matriculados')
...
@freeze_time("2025-01-28")
def test_build_xlsx_alunos_matriculados_cei_cemei(dados_planilha_alunos_matriculados_cei_cemei):
...
def test_montar_cabecalho_padrao():
    ...
    assert rows[1] == ('DRE', 'Lote', 'Tipo de unid.', 'Unid. educacional', 'Tipo de Turma', 'Período', 'Matriculados')
...
def test_montar_faixas_etarias(dados_planilha_alunos_matriculados):
    ...
    rows = list(ws.iter_rows(values_only=True))
    assert rows[0] == ('Faixa Etária', None, None, None, None, 'Alunos Matriculados', None)
