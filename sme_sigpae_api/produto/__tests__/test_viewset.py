...
from sme_sigpae_api.produto.api.serializers.serializers import ReclamacaoDeProdutoSerializer
...
def test_view_muda_status_com_justificativa_e_anexo_retorna_200(
    ...
    assert reclamacao_respondido_terceirizada.status == ReclamacaoProdutoWorkflow.RESPONDIDO_TERCEIRIZADA
    ...
    assert resposta.data == ReclamacaoDeProdutoSerializer(reclamacao_respondido_terceirizada).data
...
def test_view_muda_status_com_justificativa_e_anex_retorna_exception(
    ...
    assert resposta.data["detail"] == "Erro de transição de estado: Transition 'codae_recusa' isn't available from state 'CODAE_ACEITOU'."
...
def test_view_quantidade_reclamacoes_ativas_retorna_uma(
    ...
    reclamacoes_ativas = viewset.quantidade_reclamacoes_ativas(reclamacao_respondido_terceirizada)  
...
def test_view_quantidade_reclamacoes_ativas_retorna_nenhuma(
    ...
    reclamacao = reclamacao_respondido_terceirizada.homologacao_produto.reclamacoes.first()
...
def test_view_update_analise_sensorial_status(
    reclamacao_respondido_terceirizada
):
    analises_sensoriais = (
            reclamacao_respondido_terceirizada.homologacao_produto.analises_sensoriais.filter(
    ...
    viewset = ReclamacaoProdutoViewSet()
    viewset.update_analise_sensorial_status(analises_sensoriais, AnaliseSensorial.STATUS_RESPONDIDA) 
