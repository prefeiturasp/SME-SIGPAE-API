...
class GrupoSuspensaoAlimentacaoSerializerViewSet(viewsets.ModelViewSet):
    ...
    def escola_cancela(self, request, uuid=None):
        ...
        try:
            assert (  # nosec
                grupo_suspensao_de_alimentacao.status != grupo_suspensao_de_alimentacao.workflow_class.ESCOLA_CANCELOU
            ...
            if (
                ...
                or len(datas) + grupo_suspensao_de_alimentacao.suspensoes_alimentacao.filter(cancelado=True).count()
                == grupo_suspensao_de_alimentacao.suspensoes_alimentacao.count()
            ):
                grupo_suspensao_de_alimentacao.cancelar_pedido(user=request.user, justificativa=justificativa)
            else:
                services.enviar_email_ue_cancelar_pedido_parcialmente(grupo_suspensao_de_alimentacao)
            if hasattr(grupo_suspensao_de_alimentacao, "suspensoes_alimentacao"):
                grupo_suspensao_de_alimentacao.suspensoes_alimentacao.filter(data__in=datas).update(cancelado_justificativa=justificativa, cancelado=True)
            serializer = self.get_serializer(grupo_suspensao_de_alimentacao)
            return Response(serializer.data)
        except InvalidTransitionError as e:
            return Response(
