...
@patch("src.escola.models.DiretoriaRegional.objects.all")
@patch(
    "src.eol_servico.utils.EOLServicoSGP.matricula_por_escola",
...
def test_calendario_sgp(mock_escolas):
    with patch("src.escola.models.Escola.objects.all", return_value=mock_escolas):
        with patch.object(
            NovoSGPServico,
            "dias_letivos",
...
@patch("src.escola.services.NovoSGPServico.dias_letivos")
@patch("src.escola.utils.processa_dias_letivos")
...
def make_periodo(nome_periodo, nomes_faixas):
    ...
    return {
        ...
        "quantidade": 10,
        "faixas": [
            {"nome_faixa": nome, "dias": [], "alunos_por_faixa": []}
            for nome in nomes_faixas
        ],
    }
