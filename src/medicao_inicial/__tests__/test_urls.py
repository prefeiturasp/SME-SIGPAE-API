...
def test_url_endpoint_relatorio_adesao_exportar_xlsx_com_escolas(
    mock_exporta_xlsx,
    client_autenticado_coordenador_codae,
    categoria_medicao,
    tipo_alimentacao_refeicao,
...
def test_url_endpoint_relatorio_consolidado_xlsx_com_datas_fora_do_mes_referencia(
    ...
    monkeypatch.setattr(
        "src.medicao_inicial.api.viewsets.exporta_relatorio_consolidado_xlsx.delay",
        fake_delay,
    )
...
def test_url_endpoint_atualiza_informacoes_basicas_medicao_erro_serializer(
    client_autenticado_da_escola,
    solicitacao_medicao_informacoes_basicas,
    tipo_contagem_alimentacao,
    usuario_admin,
    ...
    assert response.json() == {
        "responsaveis": {
            "non_field_errors": ['Esperava uma lista de itens, mas recebeu tipo "int".']
        }
    }


def test_url_endpoint_atualiza_informacoes_basicas_medicao_usuario_nao_autrizado(
    client_autenticado_codae_medicao,
    solicitacao_medicao_informacoes_basicas,
    tipo_contagem_alimentacao,
):
...
def test_url_endpoint_relatorio_consolidado_com_data_final_nula(
    client_autenticado_codae_medicao,
    relatorio_financeiro_emef,
    edital,
    grupo_unidade_escolar_emef,
):
    # Cria uma parametrização financeira SEM data_final
    # para o mesmo lote e grupo do relatorio_financeiro_emef (mes=10, ano=2025)
    baker.make(
        "ParametrizacaoFinanceira",
        edital=edital,
        lote=relatorio_financeiro_emef.lote,
    ...
    response = client_autenticado_codae_medicao.get(
        f"/medicao-inicial/relatorio-financeiro/relatorio-consolidado/{relatorio_financeiro_emef.uuid}/",
        content_type="application/json",
    )

    assert response.status_code == status.HTTP_200_OK, response.json()
    assert "lote" in response.json()


def test_url_endpoint_relatorio_consolidado_sem_parametrizacao_retorna_404(
    client_autenticado_codae_medicao,
    relatorio_financeiro_emef,
):
    # Sem parametrização financeira, deve retornar 404
    response = client_autenticado_codae_medicao.get(
        f"/medicao-inicial/relatorio-financeiro/relatorio-consolidado/{relatorio_financeiro_emef.uuid}/",
        content_type="application/json",
    )
...
def test_endpoint_exportar_pdf_relatorio_financeiro_nao_encontrado(
    ...
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.data == {"Erro": "Relatório financeiro não encontrado."}
...
def test_url_endpoint_finaliza_medicao_recreio_emef(
    client_autenticado_adm_da_escola,
    escola,
