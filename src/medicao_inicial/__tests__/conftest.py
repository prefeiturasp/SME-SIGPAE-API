...
def escola_cieja(tipo_unidade_escolar_cieja):
    ...
    return baker.make(
        ...
        diretoria_regional=diretoria_regional,
        tipo_gestao=tipo_gestao,
        tipo_unidade=tipo_unidade_escolar_cieja,
        codigo_eol="111329",
    )
...
def solicitacoes_cei_relatorio_unificado(
    ...
    for dia in range(1, 31):
        for faixa in faixas_etarias_ativas:
            for campo in ["frequencia", "matriculados"]:
                baker.make(
                    "ValorMedicao",
                    dia=f"{dia:02d}",
                    nome_campo=campo,
...
def solicitacao_com_historico_correcao(django_user_model, escola):
    hist = [
        ...
        },
        {
            ...
            "alteracoes": [{"periodo_escolar": "MANHA"}],
        },
    ]
    solicitacao_medicao = baker.make(
        "SolicitacaoMedicaoInicial",
    ...
    baker.make(
        ...
        status_evento=LogSolicitacoesUsuario.MEDICAO_ENVIADA_PELA_UE,
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 10, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
    baker.make(
        "LogSolicitacoesUsuario",
        ...
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 11, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
    baker.make(
        "LogSolicitacoesUsuario",
        ...
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 12, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
    baker.make(
        "LogSolicitacoesUsuario",
        ...
        status_evento=LogSolicitacoesUsuario.MEDICAO_APROVADA_PELA_DRE,
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 13, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
    baker.make(
        "LogSolicitacoesUsuario",
        ...
        status_evento=LogSolicitacoesUsuario.MEDICAO_CORRECAO_SOLICITADA_CODAE,
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 14, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
    baker.make(
        "LogSolicitacoesUsuario",
        ...
        status_evento=LogSolicitacoesUsuario.MEDICAO_CORRIGIDA_PARA_CODAE,
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 16, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
    baker.make(
        "LogSolicitacoesUsuario",
        ...
        status_evento=LogSolicitacoesUsuario.MEDICAO_APROVADA_PELA_CODAE,
        solicitacao_tipo=LogSolicitacoesUsuario.MEDICAO_INICIAL,
        criado_em=datetime.datetime(2026, 4, 6, 17, 0, 0, tzinfo=datetime.timezone.utc),
        usuario=usuario,
    )
...
@pytest.fixture
def grupo_unidade_escolar_emei(
...
@pytest.fixture
def relatorio_financeiro_emei(
    escola_emei,
    grupo_unidade_escolar_emei,
):
    return baker.make(
        "RelatorioFinanceiro",
        grupo_unidade_escolar=grupo_unidade_escolar_emei,
        lote=escola_emei.lote,
        mes="11",
        ano="2025",
    )
...
@pytest.fixture
def parametrizacao_financeira_emei(
    edital,
    escola_emei,
    tipo_alimentacao_lanche,
    tipo_alimentacao_lanche_4h,
    ...
    parametrizacao_financeira = baker.make(
        ...
        legenda="Parametrização Financeira: Teste Grupo 3 - EMEI",
    )
...
@pytest.fixture
def grupo_unidade_escolar_cieja(
    tipo_unidade_escolar_cieja,
    tipo_unidade_escolar_cmct,
):
    return baker.make(
        "GrupoUnidadeEscolar",
        nome="Grupo 6",
...
@pytest.fixture
def relatorio_financeiro_cieja(
    escola_cieja,
    grupo_unidade_escolar_cieja,
):
    return baker.make(
        "RelatorioFinanceiro",
        grupo_unidade_escolar=grupo_unidade_escolar_cieja,
        lote=escola_cieja.lote,
        mes="03",
        ano="2026",
    )
...
@pytest.fixture
def categoria_inscritos_recreio():
    return baker.make("CategoriaAlimentacao", nome="Inscritos")


@pytest.fixture
def categoria_colaboradores_recreio():
    return baker.make("CategoriaAlimentacao", nome="Colaboradores")


@pytest.fixture
def categoria_infantil_recreio():
    return baker.make("CategoriaAlimentacao", nome="Infantil")
...
def solicitacao_recreio_emef(
    ...
    baker.make(
        ...
        categoria=categoria_inscritos_recreio,
        tipo_alimentacao=tipo_alimentacao_refeicao,
    )
    baker.make(
        "RecreioNasFeriasUnidadeTipoAlimentacao",
    ...
    baker.make(
        "RecreioNasFeriasUnidadeTipoAlimentacao",
        recreio_ferias_unidade=participante,
        categoria=categoria_colaboradores_recreio,
        tipo_alimentacao=tipo_alimentacao_refeicao,
    )
    baker.make(
        "RecreioNasFeriasUnidadeTipoAlimentacao",
...
def solicitacao_recreio_cei(
    ...
    solicitacao_recreio_nas_ferias = baker.make(
        SolicitacaoMedicaoInicial,
        mes="12",
        ano="2025",
        escola=escola_cei,
    ...
    quantidade_dieta_autorizada = 3
    frequencia_por_faixa = int(participante.num_inscritos / len(faixas_etarias_ativas))
    for offset in range(
        (recreio_nas_ferias.data_fim - recreio_nas_ferias.data_inicio).days + 1
    ):
        data = recreio_nas_ferias.data_inicio + datetime.timedelta(days=offset)
...
def solicitacao_recreio_cemei(
    ...
    tipo_alimentacao_lanche,
    tipo_alimentacao_almoco,
    grupo_recreio_nas_ferias_0_a_3_anos_e_11_meses,
    grupo_recreio_nas_ferias_4_a_14_anos,
    grupo_colaboradores,
    categoria_medicao,
    classificacao_dieta_tipo_a_enteral,
    categoria_medicao_dieta_a,
    ...
    for alimentacao in [
        ...
        baker.make(
            "RecreioNasFeriasUnidadeTipoAlimentacao",
            recreio_ferias_unidade=participante_cei,
            categoria=categoria_inscritos_recreio,
            tipo_alimentacao=alimentacao,
        )

        if alimentacao in [tipo_alimentacao_refeicao, tipo_alimentacao_sobremesa]:
            baker.make(
                "RecreioNasFeriasUnidadeTipoAlimentacao",
                recreio_ferias_unidade=participante_emei,
                categoria=categoria_infantil_recreio,
                tipo_alimentacao=alimentacao,
            )
            baker.make(
                "RecreioNasFeriasUnidadeTipoAlimentacao",
    ...
    medicao_recreio_nas_ferias_0_a_3_anos = baker.make(
        "Medicao",
        solicitacao_medicao_inicial=solicitacao_recreio_nas_ferias,
        grupo=grupo_recreio_nas_ferias_0_a_3_anos_e_11_meses,
    )

    medicao_recreio_nas_ferias_4_a_14_anos = baker.make(
        "Medicao",
        solicitacao_medicao_inicial=solicitacao_recreio_nas_ferias,
        grupo=grupo_recreio_nas_ferias_4_a_14_anos,
    )
    ...
    quantidade_dieta_autorizada = 3
    frequencia_por_faixa = int(
        participante_cei.num_inscritos / len(faixas_etarias_ativas)
    )
    for offset in range(
        (recreio_nas_ferias.data_fim - recreio_nas_ferias.data_inicio).days + 1
    ):
        ...
        baker.make(
            "ValorMedicao",
            medicao=medicao_recreio_nas_ferias_4_a_14_anos,
            categoria_medicao=categoria_medicao,
            nome_campo="participantes",
            dia=dia,
            valor=str(participante_emei.num_inscritos),
        )
        baker.make(
            "ValorMedicao",
            medicao=medicao_recreio_nas_ferias_0_a_3_anos,
            categoria_medicao=categoria_medicao,
            nome_campo="participantes",
            dia=dia,
            valor=str(participante_cei.num_inscritos - quantidade_dieta_autorizada),
        )
        baker.make(
            "ValorMedicao",
            medicao=medicao_colaboradores,
            ...
            dia=dia,
            valor=str(
                participante_emei.num_colaboradores + participante_cei.num_colaboradores
            ),
        )
        dieta = baker.make(
            "LogQuantidadeDietasAutorizadasRecreioNasFerias",
