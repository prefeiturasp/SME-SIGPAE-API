...
def gera_xls_relatorio_produtos_homologados_async(
    ...
    try:
        from src.produto.api.viewsets import (
...
def gera_excel_relatorio_reclamacao_produtos_async(
    ...
    try:
        arquivo = relatorio_reclamacao_produtos_excel(
...
def gera_pdf_relatorio_produto_async(
    user: str,
    nome_arquivo: str,
    uuid_produto: str,
) -> None:
    ...
    try:
        produto = Produto.objects.get(uuid=uuid_produto)
        response = relatorio_produto_homologacao(
            request=None,
            produto=produto,
        )
        ...
        atualiza_central_download_com_erro(obj_central_download, str(e))
        logger.error(f"Erro ao gerar relatório do produto: {e}")
