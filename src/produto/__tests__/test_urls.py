...
def test_url_endpoint_criar_item_cadastro_e_marca(client_autenticado_vinculo_escola_ue):
    from src.produto.models import ItemCadastro
...
def test_fluxo_correcao_dados_produto_homologado(
    ...
    homologacao_produto.reclamacao = reclamacao_respondido_terceirizada
    ...
    assert hom_copia.rastro_terceirizada is not None
    assert hom_copia.eh_copia is True
    assert hom_copia.reclamacoes.count() > 0
...
def test_fluxo_correcao_dados_produto_nao_homologado(
    ...
    response = client.patch(
        f"/homologacoes-produtos/{homologacao_produto_pendente_homologacao.uuid}/alteracao-produto-homologado/",
        content_type="application/json",
        data=json.dumps(data),
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_url_endpoint_produtos_relatorio_reclamacoes_excel(
    client_autenticado_vinculo_terceirizada, hom_produto_com_editais
):
    client = client_autenticado_vinculo_terceirizada[0]
    response = client.get(
...
def test_url_endpoint_produtos_relatorio_reclamacoes_excel_edital_nao_enviado(
    client_autenticado_vinculo_terceirizada, hom_produto_com_editais
):
    client = client_autenticado_vinculo_terceirizada[0]
    response = client.get(
        "/produtos/relatorio-reclamacao-excel/", content_type="application/json"
    )
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {
        "detail": "É obrigatório selecionar pelo menos um edital para gerar o relatório."
...
def test_alteracao_dados_produto_homologado_erro_copia(
    client_autenticado_vinculo_terceirizada,
...
def test_url_endpoint_homologacao_produto_codae_suspendeu(
    ...
    response = client_autenticado_vinculo_codae_produto.get(
        "/painel-gerencial-homologacoes-produtos/filtro-por-status/codae_suspendeu/"
    )
    assert response.status_code == status.HTTP_200_OK
