...
class DadosUsuarioEOLCompletoViewSet(ViewSet):
    ...
    def retrieve(self, request, registro_funcional=None):
        try:
            ...
            codigo_ue_dre_usuario_logado = serializer_usuario_logado.data[
                "vinculo_atual"
            ]["instituicao"]["codigo_eol"]
            codigo_ue_rf_buscado = dados_usuario["cargos"][0]["codigoUnidade"]
            codigo_dre_rf_buscado = dados_usuario["cargos"][0]["codigoDre"]

            if not usuario_eh_codae and (
                codigo_ue_dre_usuario_logado != codigo_ue_rf_buscado
                and codigo_ue_dre_usuario_logado != codigo_dre_rf_buscado
            ):
                nome_instituicao = serializer_usuario_logado.data["vinculo_atual"][
                    "instituicao"
                ]["nome"]
