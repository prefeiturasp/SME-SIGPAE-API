...
def test_template_email_base():
    ...
    assert (
        f"O fornecedor <strong>{nome_empresa}</strong> corrigiu e enviou os Documentos de Recebimento referente ao Cronograma <strong>Nº {numero_cronograma}</strong> do Produto <strong>{nome_produto}</strong>"
        in html
    )
