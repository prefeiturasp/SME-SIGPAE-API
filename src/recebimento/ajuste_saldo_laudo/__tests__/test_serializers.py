...
class TestAjusteSaldoDetalharSerializer:
    def test_serializer_campos(self, ajuste_saldo):
    ...
    def test_serializer_valores(self, ajuste_saldo):
        ...
        assert data["uuid"] == str(ajuste_saldo.uuid)
        assert (
            data["numero_cronograma"]
            == ajuste_saldo.documento_recebimento.cronograma.numero
        )
        assert data["numero_laudo"] == ajuste_saldo.documento_recebimento.numero_laudo
        assert (
            data["unidade_medida"]
        ...
        assert data["quantidade_descontada"] == str(ajuste_saldo.quantidade_descontada)
