...
def limpar_campos_numericos(apps, _):  # noqa: C901
    ...
    for obj in Veiculo.objects.all():
        ...
        if obj.embalagens_nota_fiscal:
            try:
                valor = str(obj.embalagens_nota_fiscal)
                obj.embalagens_nota_fiscal = int(float(valor.replace(",", ".")))
            except ValueError:
                obj.embalagens_nota_fiscal = None

        if obj.embalagens_recebidas:
            try:
                valor = str(obj.embalagens_recebidas)
                obj.embalagens_recebidas = int(float(valor.replace(",", ".")))
            except ValueError:
