...
from src.produto.models import InformacaoNutricional
from src.produto.utils.relatorio_reclamacao_produto import (
...
def relatorio_historico_produto(request, produto):
    valida_request_method_get(request)
...
def _obter_valor(tabela, indice_valor, indice_principal):
    try:
        return tabela["valores_campos"][-1][indice_valor + 1]
    except IndexError:
        return tabela["valores_campos"][-1][indice_principal + 1]
...
def relatorio_historico_ocorrencias_medicao_inicial(
    ...
    logo_sme = convert_image_to_base64(
        "src/relatorios/static/images/LOGO_FUNDO_CLARO.png", "png"
    )
