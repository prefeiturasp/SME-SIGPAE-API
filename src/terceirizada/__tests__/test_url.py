...
@pytest.mark.parametrize(
    "params, expected_count, excluded_numbers",
    [
        ("", 3, []),
        ("?excluir_encerrados=true", 2, ["EDITAL-002"]),
        ("?excluir_parceira=true", 2, ["PARCEIRA"]),
        (
            "?excluir_encerrados=true&excluir_parceira=true",
            1,
            ["EDITAL-002", "PARCEIRA"],
        ),
    ],
)
def test_lista_numeros_query_params_variados(
    client_autenticado_terceiro_com_filtros, params, expected_count, excluded_numbers
):
